...
exports('regist', function (params, done) {
  $('.registrationWrapper').empty(); // Switching checkmark

  function onCheckmark(input) {
    input.parent().parent().addClass('corrected').removeClass('uncorrected');
  }

  function offCheckmark(input) {
    input.parent().parent().addClass('uncorrected').removeClass('corrected');
  }
  ...
  if (params.fast == 'fast') {
    insertHtmlModules({
      ".registrationWrapper": ["registration/fast.html"]
    }, function () {
      // Preloader finishes
        ...
        if (cur.val().length >= 8 || cur.parent().parent().is('.corrected') || cur.parent().parent().is('.uncorrected')) {
          if (/^([A-Za-z0-9]{8,})$/.test(cur.val())) {
            ...
            if ($('#PasswordSecondary').val() != cur.val() && $('#PasswordSecondary').val().length > 1) {
              ...
              if ($('#PasswordSecondary').val() == cur.val()) $('#PasswordSecondary').parent().addClass('corrected').removeClass('uncorrected');
            }
      ...
      $('#PasswordSecondary').on('input', function (event) {
        $("[data-id=nextButton]").addClass('disable');
        var cur = $(event.target);

        if (cur.val().length >= 8 || cur.parent().parent().is('.corrected') || cur.parent().parent().is('.uncorrected')) {
          if (cur.val() == $('#Password').val()) {
            cur.parent().addClass('corrected').removeClass('uncorrected');
            ...
            cur.parent().removeClass('corrected').addClass('uncorrected');
            $("[data-id=nextButton]").addClass('disable');
          }
        }
      });
      $('#PasswordSecondary').on('blur', function (event) {
        var cur = $(event.target);
        if (cur.val().length < 8) offCheckmark(cur);
      }); // Email
      ...
      $('#Email').on('input', function (event) {
        ...
        if (cur.val().length >= 6 || cur.parent().is('.corrected') || cur.parent().is('.uncorrected')) {
          if (/^(([^<>()\[\]\.,;:\s@\"]+(\.[^<>()\[\]\.,;:\s@\"]+)*)|(\".+\"))@(([^<>()[\]\.,;:\s@\"]+\.)+[^<>()[\]\.,;:\s@\"]{2,})$/.test(cur.val().toLowerCase())) {
            cur.parent().removeClass('uncorrected').addClass('corrected');
      ...
      $("[data-id=nextButton]").on('click', function () {
    ...
    insertHtmlModules({
      ".registrationWrapper": ["registration/details.html", "registration/information.html", "registration/confirmation.html"]
    }, function () {
      // Preloader finishes
      if ($('#page-preloader').data("status") != 'done') {
        $('#page-preloader').addClass('done');
      ...
      $("[data-id=details]").addClass('active');
      $('.button.primary.disable').removeClass('disable');
      $('.button.primary.disable').addClass('backward'); // curPopup handler
      ...
      Popup.registerListener(function (val) {
        $('input').on('blur', function (event) {// console.log(`blur`);
          // window.scrollTo(0, 0);
        });
        ...
        if (val == 'details') {
          ...
          if (val == 'information') {
            ...
            $('#Country').on('focusout', function (event) {
              var cur = $(event.target);
              cur.parent().removeClass('uncorrected').addClass('corrected');
            ...
            $('#Currency').on('focusout', function (event) {
              var cur = $(event.target);
              cur.parent().removeClass('uncorrected').addClass('corrected');
            ...
            $('#SecondName').on('change', function (event) {
              firstEnter++;
            });
            $('#SecondName').on('input', function (event) {
              var cur = $(event.target);
              var curNeighbour = $('#FirstName');
            ...
            $('#Day').on('focusout', function (event) {
              date.day = true;
              var cur = $(event.target);
            ...
            $('#Month').on('focusout', function (event) {
              date.month = true;
              var cur = $(event.target);
            ...
            $('#Year').on('focusout', function (event) {
              date.year = true;
              var cur = $(event.target);
            ...
            $('#Adress').on('input', function (event) {
              var cur = $(event.target);
            ...
            if (!$("[data-class=gender]").eq(0).is('checked') || !$("[data-class=gender]").eq(1).is('checked')) {
              $(event.target).parent().removeClass('corrected').addClass('uncorrected');
            }

            $("[data-class=gender]").on('click', function (event) {
              var cur = $(event.target);
              onCheckmark(cur);
      ...
      Popup.active = 'details'; // Preloader finishes
      ...
      $("[data-id=nextButton]").on('click', function () {
        if (Popup.active == 'details') {
          ...
          if (Popup.active == 'information') {
            ...
            var url = 'http://bestline.bet/api/?key=inplay';
            var data = JSON.stringify(RegistrationForm);
      ...
      $("[data-id=prevButton]").on('click', function () {
        if (Popup.active == 'information') {
          $("[data-id=information]").removeClass('active');
          $("[data-id=information]").addClass('not-active');
          $("[data-id=details]").removeClass('not-active');
